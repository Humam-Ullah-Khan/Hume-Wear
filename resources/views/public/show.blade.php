        ...
        <div class="w-full lg:w-1/2 flex flex-col">
            ...
            <div class="space-y-3 mb-6">
                @php
                    ...
                    $productCode = $product->unique_code ?? 'HE-' . str_pad($product->id, 3, '0', STR_PAD_LEFT);
                    $whatsappMsg = "Hi! I'd like to order this product.\n\n";
                    $whatsappMsg .= "*Product:* {$product->title}\n";
                    $whatsappMsg .= "*Code:* {$productCode}\n";
                    $whatsappMsg .= "*Price:* PKR " . number_format($finalPrice, 0) . "\n";
                    $whatsappMsg .= "*Image:* {$mainImage}\n\n";
                    $whatsappMsg .= "Please confirm availability. Thank you!";
                @endphp
                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $siteSettings['whatsapp']) }}?text={{ urlencode($whatsappMsg) }}" target="_blank"
                   class="btn-hover flex items-center justify-center gap-3 w-full bg-[#25D366] text-white px-8 py-4 text-sm uppercase tracking-[0.2em] font-bold hover:bg-[#20bd5a] transition rounded">
